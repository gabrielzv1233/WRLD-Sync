...
## 2026-08-31 Manifest / preprocessing / HuBERT FA continuation pass

- Added a shared root-level `models_manifest.json` catalog used by the backend and Models UI for Whisper, Qwen3 ASR / Forced Aligner, NVIDIA Parakeet, HuBERT FA, and UVR separator metadata. Model-specific runtime adapters remain explicit rather than pretending every model has the same API.
- Extended the existing project-local Qwen/Parakeet model manager instead of replacing it. Hugging Face installs now download into persistent `models/.partials/<model>/` paths with HTTP Range resume, preserve partial bytes on pause/restart, verify every required file against Hugging Face metadata before publishing the install, and atomically move a verified model into its final project folder.
- Removed a resumable-download performance bug where the entire partial model tree was recursively rescanned after every 1 MiB network chunk. Download progress now uses a running byte counter and only scans partial files when a paused-state percentage must be reconstructed.
- Added Task Queue pause/resume/cancel integration for managed model downloads. Pending pause/resume now tracks live asyncio queue tickets so the same task cannot be accidentally enqueued twice and rerun after completion. Destructive cancel removes project-local Hugging Face partials and HuBERT FA partials.
- Added Advanced processing settings for optional UVR vocal isolation, separator model selection, yt-dlp extraction, and High / Medium / Small URL-audio quality. Cookies remains visible but intentionally disabled because browser/auth cookie import is not implemented in this pass.
- Added optional cached all-vocals preprocessing through `audio-separator`. Stem cache identity is based on the canonical decoded-audio SHA-256 plus separator model/target configuration, so changing the transcription or alignment model does not rerun UVR.
- Added per-track `Upload Vocals` references in the bottom editor controls. Uploaded/URL references are normalized to cached audio-only FLAC, persisted in SQLite for catalog songs or local-audio hashes, take priority over UVR, and automatically clear stale DB associations when their cached file disappears.
- Added yt-dlp extraction for YouTube, YouTube Music, and SoundCloud while retaining the existing direct HTTP path for ordinary direct-audio URLs. Downloaded remote media is normalized through the existing local-audio registration path before processing.
- Queue tasks now snapshot preprocessing settings (`preprocess_vocals`, separator model/target) at enqueue time, and completed processed-lyrics settings persist the actual `analysis_source` recipe used by that job.
- Replaced the native audio controls with the production custom player while keeping the real `<audio>` element and normal document flow. Seek/volume fill CSS and JavaScript now consistently use `--seek-pct` / `--volume-pct`; mute preserves the previous volume and moving the slider unmutes.
- Added the Models catalog UI with Curated / All models, task filters, Material Symbols Rounded search glyph, install/remove state, in-place active-download progress, and a Model Guide. The guide uses Speed, Vocal accuracy, VRAM load, and Vocal isolation benefit terminology, explains that values are comparative public-information estimates rather than local benchmarks, provides tag/stat tooltips, and has smoothly expanding Technical Details.
- Added HuBERT FA combined to the experimental Sync-model selector and model manager. Its pinned v0.0.7 installer resolves the release asset from GitHub release metadata and uses GitHub's published SHA-256 digest when available instead of relying on an unverified hardcoded checksum.
- Implemented the HuBERT FA English lyric-to-phoneme bridge. Visible lyric spelling/punctuation is preserved exactly while hidden alignment metadata uses the upstream HuBERT dictionary first, direct CMUdict lookup, curated compact slang pronunciations, context-aware neural G2P for unknown/slang/context-sensitive terms, and controlled singing variants.
- HuBERT singing tolerance remains deliberately bounded: candidate generation allows small repeated vowel/glide events and a limited unstressed-vowel elision rather than unrestricted phoneme invention. This covers re-articulation cases such as a sung `unknown` behaving more like `unknowowowown` without making alignment confidence meaningless.
- HuBERT FA now runs the ONNX acoustic pass once, reuses those logits to score plausible per-word pronunciation candidates against local audio windows, applies a small complexity penalty to gratuitous inserted events, then runs the final full-song forced alignment with the selected pronunciations.
- HuBERT pronunciation candidate scoring uses detached one-word plans, so testing an alternate pronunciation cannot temporarily corrupt the global original-word-to-phoneme range map. Final timings are reconstructed onto the original lyric tokens rather than exposing ARPAbet/phoneme text or requiring user-facing `.lab` files.
